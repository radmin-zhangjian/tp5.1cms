...
class Admin extends Model
{
    ...
    // 设置只读字段
    protected $readonly = ['adm_name']; // 不可更新
     ...
     * @access public
     */
    public function getIsEffectAttr($value, $data)
    {
        $status = [0 => '无效', 1 => '有效'];
        // return $status[$value];
        return $status[$data['is_effect']];
    }
    /**
     * is_delete
     * @access public
     */
    public function getIsDeleteAttr($value, $data)
    {
        $status = [0 => '正常', 1 => '删除'];
        // return $status[$value];
        return $status[$data['is_delete']];
    }
